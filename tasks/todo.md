...
- [x] **DNS held by K13**, the Aug 1 existential risk is closed and the cutover needs no client scheduling
...
- [ ] **4.** Point `lobsterlab.us` DNS at Vercel. **K13 holds the DNS**, so this needs no client
      scheduling and no waiting. The cutover is entirely in our hands.
- [ ] **4b. Delete `public/soundcheck.html`.** It is a K13 stakeholder document living inside the
      client's site. Harmless while the whole domain is noindexed, but once lobsterlab.us is live
      and indexable it would be a public page asking the client for their revenue figures.
